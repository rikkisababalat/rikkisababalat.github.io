        ...
        window.onload = () => {
            ...
            if (diagnoses.length > 0) {
                // Set default view to the highest scored diagnosis
                const bestDiagnosis = diagnoses.reduce((prev, current) => (prev.matchScore > current.matchScore) ? prev : current);
                renderDetails(bestDiagnosis.id);
            }
            createChart();
            loadQuiz();
        };